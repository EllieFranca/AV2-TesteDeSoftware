...
document.getElementById('login-link1').addEventListener('click', function (evnt1) {
...
telefone.addEventListener('keyup', () => {
    ...
    if (valor.length > 7) {
        valor = valor.substring(0, 10) + '-' + valor.substring(11, 14);
    }
...
function cadastrar(event) {
    ...
    if (validEmail && validSenha && validNome && validIdade && validTel) {
        ...
        const emailExiste = ListaUser.some(user => user.eailCad === email.value);
        if (emailExiste) {
...
function logar(event) {
    ...
    if (email2.value == userValid.emailLog && password2.value == userValid.senhaLog) {
        ...
        setTimeout(() => {
            ...
            window.location.href = 'index-homepage.js';
        }, 1000);
        limparCampos()
    } else {
